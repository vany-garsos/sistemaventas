...
@section('content')
    ...
    <div class="container-fluid px-4">
        ...
        <div class="mb-4">
            <a href="{{ route('proveedores.create') }}"><button type="button" class="btn btn-primary">Añadir un nuevo
                    registro</button></a>
        </div>
                    ...
                    <tbody> 
                        @foreach($proveedores as $proveedore)
                           <tr>
                            ...
                            <div class="modal fade" id="confirmEliminar-{{ $proveedore->persona->id }}" tabindex="-1"
                                aria-labelledby="confirmModal" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h1 class="modal-title fs-5" id="exampleModalLabel">
                                                {{ $proveedore->persona->estado == 1 ? 'Eliminar proveedor' : 'Restaurar proveedor' }}
                                            </h1>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            {{ $proveedore->persona->estado == 1 ? '¿Seguro deseas eliminar este proveedor?' : '¿Deseas restaurar el proveedor?' }}
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary"
                                                data-bs-dismiss="modal">Cerrar</button>

                                            <form action="{{ route('proveedores.destroy', $proveedore->persona->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="btn btn-danger">{{ $proveedore->persona->estado == 1 ? 'Sí, eliminar' : 'Sí, restaurar' }}</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
